# Arch Linux Installation Guide - Intel Hardware
...
- **Minimal Window Manager**: Wayland compositor setup with Intel graphics drivers
- **Intel CPU Microcode**: Intel processor microcode updates for security and stability
- **Intel Graphics Drivers**: Mesa, Vulkan, and VA-API drivers for Intel integrated graphics
- **Academic Documentation**: Comprehensive theoretical foundations, references, and citations

**Target Audience:**
- System administrators installing Arch Linux on Intel-based hardware
- Users with Intel processors and integrated graphics
- Educational institutions teaching Linux system administration on Intel hardware
- Researchers documenting Linux installation procedures for Intel systems

**Prerequisites:**
- Intel x86-64 compatible processor
- Intel integrated graphics (HD Graphics, Iris, Arc) or Intel discrete graphics
...
- Second computer for remote SSH installation (highly recommended, optional)
...
**Note:** This phase installs Intel graphics drivers. For other hardware configurations, refer to:
...
### Step 12.6: Install Intel Graphics Drivers

#### 12.6.1 Theoretical Foundation of Intel Graphics Drivers

**Intel Graphics Architecture:**

Intel integrated graphics are built into Intel processors and share system memory with the CPU. Intel provides open-source graphics drivers through the Mesa project, which implements OpenGL, Vulkan, and other graphics APIs.
...
- `intel-media-driver` - Hardware video acceleration (VA-API) for Intel Gen8+ (Broadwell and newer)
  - [Intel Media Driver GitHub](https://github.com/intel/media-driver)
  - [ArchWiki Hardware Video Acceleration](https://wiki.archlinux.org/title/Hardware_video_acceleration)
- `libva-intel-driver` - Legacy VA-API driver (for older Intel GPUs, Gen7 and older)
  - [VA-API Documentation](https://github.com/intel/libva)

**Note:** For Intel Gen8+ (Broadwell 2015 and newer), use `intel-media-driver`. For older Intel GPUs (Gen7 and older), use `libva-intel-driver`. Both can be installed, but `intel-media-driver` takes precedence on supported hardware.
...
**Last Updated:** December 2, 2025  
**Author:** Arch Linux Installation Guide - Intel Hardware  
...
Intel microcode updates are automatically loaded by the kernel during boot. The `intel-ucode` package provides microcode updates that:
...
For Intel graphics, early KMS enables graphics during boot (before user space starts). This is configured in `/etc/mkinitcpio.conf`:
...
# Change to (add i915 for Intel graphics):
MODULES=(i915)
...
**i915** is the kernel module for Intel integrated graphics (HD Graphics, Iris, Arc).

**Official Resources:**
- [ArchWiki Intel Graphics](https://wiki.archlinux.org/title/Intel_graphics)
- [Intel Graphics Driver Documentation](https://01.org/linuxgraphics/)
